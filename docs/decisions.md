...
- **却下B：Anthropic API での LLM 構造化抽出**。従量課金キーが必須で、運用者が API 課金しない方針のため不成立。
  （検討 PoC はローカルの別ブランチ `feat/llm-extraction-poc` に未マージで退避（未 push）。将来課金可なら流用可。）
...
- **本リポの SQLite 差分検知（§4・§7）は当面休眠**（自動再スクレイピング前提の仕組みのため）。差分検知の
  概念は登録API側（Firestore）で新たに実装する方針で、`content_fingerprint()` から `games`（手動運用では
  常に空）を除いた `content_hash` を冪等 upsert に用いる。
- **将来の自動化**は「新着告知 URL の検知層」（一覧リンクの差分のみ。中身は解析しない＝年度変動に強い）として
  再導入しうる。手書きパーサ地獄には戻さない。

スキーマ仕様（抽出出力＝登録API入力＝Firestore ドキュメント）と抽出ルールの正典は、アプリ側リポ `app` の
`docs/schema-spec.md` に集約した。ドメインモデル `models.py` の enum 群を語彙として厳密に一致させる。

## 10. 積み残し（未調査）

> 注: §9 の転換により、以下のうち**パーサ拡張・個別試合充填に類する項目は凍結**する（本リポでは追わない）。
> 該当領域は手動抽出＋登録API側で必要に応じてカバーする。規約確認・サイト調査系は引き続き有効。

...
本リポジトリ（スクレイパー）の技術・設計上の決定を集約する。主要な合意日は 2026-07-05（以降の追補日は各節見出しに記載）。
...
- **却下B：Anthropic API での LLM 構造化抽出**。従量課金キーが必須で、運用者が API 課金しない方針のため不成立。
  検討 PoC はローカル未 push の別ブランチ `feat/llm-extraction-poc` に退避してあり、将来課金可なら流用できる。
...
スキーマ仕様（抽出出力＝登録API入力＝Firestore ドキュメント）と抽出ルールの正典は、アプリ側リポ（アンブレラ
配下 `app/`、private）の `docs/schema-spec.md` に集約した。ドメインモデル `models.py` の enum 群を語彙として
厳密に一致させる。

## 10. 積み残し（未調査）

> 注: §9 の転換により、以下のうち**パーサ拡張・個別試合充填に類する項目は凍結**する（本リポジトリでは追わない）。
